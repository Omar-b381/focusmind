...
import { Play, Pause, Square, Sparkles, Coffee, BatteryCharging } from 'lucide-react'
import Card from '../components/ui/Card'
import Button from '../components/ui/Button'
import { CircularProgress } from '../components/ui/Progress'

export default function FocusMode() {
  const { status, elapsed, duration, type, start, pause, reset, setSession, addMinutes } = useFocusStore()
      ...
      <div className="flex gap-2 p-1.5 bg-[#1a1d27] border border-[#2d3252]/50 rounded-2xl w-full max-w-md">
        {sessionConfigs.map((config) => {
          ...
          return (
            <button
              key={config.type}
              onClick={() => setSession(config.type, config.duration, selectedTaskId)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full items-start">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="p-5 space-y-4">
            ...
            <div className="p-3.5 bg-[#141621] border border-[#2d3252] rounded-xl flex items-center justify-between">
              <span className="text-xs text-white font-medium">تجهيز عرض التصميم لعميل FocusMind</span>
              <span className="text-[10px] text-indigo-400 font-bold">نشط 🎯</span>
            </div>
            <div className="flex gap-2">
              <Button variant="secondary" size="sm" className="w-full text-xs font-tajawal">تغيير المهمة</Button>
            </div>
          </Card>
